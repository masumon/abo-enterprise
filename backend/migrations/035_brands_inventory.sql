...
-- Idempotent and non-destructive. Existing product stock is preserved.
...
CREATE INDEX IF NOT EXISTS ix_inventory_movements_product_created ON inventory_movements(product_id, created_at DESC);
CREATE INDEX IF NOT EXISTS ix_inventory_movements_type ON inventory_movements(movement_type);

-- Baseline snapshot only; this is explicitly not fabricated historical sales.
...
SELECT p.id, 'opening_balance', p.stock_quantity, 0, p.stock_quantity, 'migration',
       'Opening inventory balance captured when the dedicated inventory module was introduced'
FROM products p
WHERE p.is_deleted = FALSE
  AND NOT EXISTS (
      SELECT 1 FROM inventory_movements m
      WHERE m.product_id = p.id AND m.movement_type = 'opening_balance' AND m.reference_type = 'migration'
  );
...
DROP TRIGGER IF EXISTS trg_products_stock_inventory ON products;
