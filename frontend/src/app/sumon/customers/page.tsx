          ...
          <div className="mx-auto max-w-5xl admin-card bg-white min-h-[80vh]">
            ...
            {detailLoading ? <div className="p-16 flex justify-center"><RefreshCw className="w-6 h-6 animate-spin text-gray-400" /></div> : (
              <div className="p-5 space-y-6">
                ...
                <section className="admin-card p-5"><h3 className="font-semibold mb-4">{bn ? "ইতিহাস" : "History"}</h3>{history && <div className="space-y-6"><div><h4 className="text-sm font-medium mb-2">{bn ? "অর্ডার" : "Orders"}</h4>{history.orders.length ? <div className="overflow-x-auto"><table className="table-premium min-w-[620px]"><thead><tr><th>No.</th><th>Status</th><th>Payment</th><th>Total</th><th>Date</th></tr></thead><tbody>{history.orders.map((item) => <tr key={item.id}><td>{item.order_number}</td><td>{item.status}</td><td>{item.payment_status}</td><td>{formatPrice(item.total)}</td><td>{item.created_at ? new Date(item.created_at).toLocaleString("en-BD") : "—"}</td></tr>)}</tbody></table></div> : <p className="text-sm text-gray-500">{bn ? "কোনো অর্ডার নেই" : "No orders"}</p>}</div><div><h4 className="text-sm font-medium mb-2">{bn ? "বুকিং" : "Bookings"}</h4>{history.bookings.length ? <div className="overflow-x-auto"><table className="table-premium min-w-[620px]"><thead><tr><th>No.</th><th>Status</th><th>Payment</th><th>Total</th><th>Date</th></tr></thead><tbody>{history.bookings.map((item) => <tr key={item.id}><td>{item.booking_number}</td><td>{item.status}</td><td>{item.payment_status || "—"}</td><td>{formatPrice(item.total)}</td><td>{item.created_at ? new Date(item.created_at).toLocaleString("en-BD") : "—"}</td></tr>)}</tbody></table></div> : <p className="text-sm text-gray-500">{bn ? "কোনো বুকিং নেই" : "No bookings"}</p>}</div><div><h4 className="text-sm font-medium mb-2">{bn ? "লিড" : "Leads"}</h4>{history.leads.length ? <div className="overflow-x-auto"><table className="table-premium min-w-[520px]"><thead><tr><th>Status</th><th>Source</th><th>Date</th></tr></thead><tbody>{history.leads.map((item) => <tr key={item.id}><td>{item.status}</td><td>{item.source}</td><td>{item.created_at ? new Date(item.created_at).toLocaleString("en-BD") : "—"}</td></tr>)}</tbody></table></div> : <p className="text-sm text-gray-500">{bn ? "কোনো লিড নেই" : "No leads"}</p>}</div></div>}</section>
              </div>
            )}
          </div>
        </div>
