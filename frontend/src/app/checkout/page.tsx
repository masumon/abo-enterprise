...
export default function CheckoutPage() {
  ...
  const paymentConsequence = (opt: { gateway: string; label: string; isManual: boolean }) => {
    if (opt.gateway === "cod" || opt.gateway === "cash_on_delivery") {
      return lang === "bn" ? "রাইডারকে দিন। এখন কিছু দিতে হবে না।" : "Pay the rider. Nothing to pay now.";
    }
    if (opt.isManual) {
      return lang === "bn" ? "Send Money করে TrxID দিন — এই পাতাতেই।" : "Send Money, then enter the TrxID — you stay on this page.";
    }
  ...
  const deliveryChargeResult = comboFreeDelivery
    ? 0
  ...
  const onSubmit = async (data: FormData) => {
    if (!pricingSettingsReady) {
      setSubmitError(lang === "bn" ? "ডেলিভারি/অগ্রিম পেমেন্টের সেটিংস অসম্পূর্ণ। অ্যাডমিন সেটিংস ঠিক না হওয়া পর্যন্ত অর্ডার নেওয়া যাবে না।" : "Delivery/advance settings are incomplete. The order cannot be placed until Admin settings are configured.");
      return;
    }
    if (isOffline()) {
  ...
  if (!hydrated || (items.length === 0 && combos.length === 0)) {
    return <div className="min-h-screen flex items-center justify-center"><div className="w-8 h-8 border-4 border-brand-200 border-t-brand-600 rounded-full animate-spin" /></div>;
  }
  ...
  return (
    <div className="min-h-screen page-surface pb-mobile-float lg:pb-8">
      <PageHero
        ...
        breadcrumbs={[{ label: lang === "bn" ? "হোম" : "Home", href: "/" }, { label: lang === "bn" ? "কার্ট" : "Cart", href: "/cart" }, { label: lang === "bn" ? "চেকআউট" : "Checkout" }]}
        variant="light"
      />
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <ol className="flex gap-2 mb-6" aria-label={lang === "bn" ? "চেকআউটের ধাপ" : "Checkout steps"}>
          {[
            ...
            return <li key={step.key} className="flex-1 grid gap-1.5"><span aria-hidden className={cn("h-[3px] rounded-full", step.done ? "bg-green-600" : active ? "bg-brand-600" : "bg-gray-200 dark:bg-white/10")} /><span className={cn("text-[11px] text-center", step.done ? "text-green-700 dark:text-green-400" : active ? "font-bold text-heading" : "text-muted")}>{lang === "bn" ? step.label.bn : step.label.en}</span></li>;
          })}
        </ol>
        ...
        {!signedIn && (
          <div className={cn("mb-6", signInRequired ? "alert-warning" : "alert-info")} role={signInRequired ? "alert" : undefined}>
            <div className="flex flex-wrap items-center gap-3">
              <p className="flex-1 min-w-[14rem] text-sm">{signInRequired ? (lang === "bn" ? "অর্ডার করতে সাইন ইন করুন। ফোন ও ইমেইল দিন — কোডটি ইমেইলে যাবে।" : "Sign in to place this order. Your phone and email — the code arrives by email.") : (lang === "bn" ? "সাইন ইন করলে পরের বার ঠিকানা নিজেই বসবে আর অর্ডার ট্র্যাক করা যাবে। না করলেও অর্ডার করতে পারবেন।" : "Sign in and your address fills itself next time, and you can track the order. You can also order without it.")}</p>
              <div className="flex gap-2">
                <Link href="/login?redirect=/checkout" className={cn("btn btn-sm", signInRequired ? "btn-primary" : "btn-outline")}>{lang === "bn" ? "সাইন ইন" : "Sign in"}</Link>
                {!signInRequired && guestAllowed && <button type="button" className="btn btn-sm btn-outline" onClick={() => document.getElementById("checkout-form")?.scrollIntoView({ behavior: "smooth", block: "start" })}>{lang === "bn" ? "অতিথি হিসেবে চালিয়ে যান" : "Continue as guest"}</button>}
              </div>
            </div>
          </div>
        )}
        ...
        {(!pricingSettingsReady) && (
          <div role="alert" className="mb-6 alert-error flex items-start gap-2">
            <AlertCircle className="w-4 h-4 flex-shrink-0 mt-0.5" />
            <span>{lang === "bn" ? "ডেলিভারি বা অগ্রিম পেমেন্টের Admin সেটিংস অসম্পূর্ণ। সঠিক চার্জ নির্ধারণ না হওয়া পর্যন্ত অর্ডার নেওয়া বন্ধ রাখা হয়েছে।" : "Delivery or advance-payment Admin settings are incomplete. Ordering is paused until the correct charges are configured."}</span>
          </div>
        )}
            ...
            <form id="checkout-form" onSubmit={handleSubmit(onSubmit)} className="space-y-6">
              ...
              <section className="enterprise-card p-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    <div><label className="form-label" htmlFor="customer-phone">{lang === "bn" ? "মোবাইল *" : "Mobile *"}</label><div className="flex gap-2"><div className="w-24"><CountrySelector selected={selectedCountry} onChange={setSelectedCountry} /></div><input id="customer-phone" {...register("customer_phone")} type="tel" className={cn("input flex-1", errors.customer_phone && "input-error")} placeholder={selectedCountry === "BD" ? "01XXXXXXXXX" : "+..."} /></div>{errors.customer_phone && <p className="text-red-500 text-xs mt-1">{errors.customer_phone.message}</p>}</div>
                    <div><label className="form-label" htmlFor="customer-email">{lang === "bn" ? "ইমেইল" : "Email"}</label><input id="customer-email" {...register("customer_email")} type="email" className="input" placeholder="your@email.com" /></div>
                  </div>
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    <div><label className="form-label" htmlFor="district">{lang === "bn" ? "জেলা *" : "District *"}</label><select id="district" {...register("district")} className={cn("input", errors.district && "input-error")}>{BD_DISTRICTS.map((d) => <option key={d} value={d}>{d}</option>)}</select>{errors.district && <p className="text-red-500 text-xs mt-1">{errors.district.message}</p>}</div>
                    <div><label className="form-label" htmlFor="upazila">{lang === "bn" ? "থানা / উপজেলা" : "Upazila / Thana"}</label><select id="upazila" {...register("upazila")} className="input"><option value="">{lang === "bn" ? "বাছাই করুন" : "Select"}</option>{upazilaOptions.map((u) => <option key={u} value={u}>{u}</option>)}</select></div>
                  </div>
                  ...
                  <div className="border-t border-gray-100 dark:border-white/10 pt-4">
                    <button type="button" onClick={() => setCompanyOpen((v) => !v)} aria-expanded={companyOpen} aria-controls="checkout-company-fields" className="w-full flex items-center justify-between gap-2 text-sm font-semibold text-heading py-1"><span className="flex items-center gap-2"><Building2 aria-hidden className="w-4 h-4 text-brand-600" />{lang === "bn" ? "প্রতিষ্ঠানের নামে ইনভয়েস" : "Invoice to a company"}</span><ChevronDown aria-hidden className={cn("w-4 h-4 motion-safe:transition-transform", companyOpen && "rotate-180")} /></button>
                    <div id="checkout-company-fields" hidden={!companyOpen} className="space-y-3 mt-3">
                      <div><label className="form-label" htmlFor="co-name">{lang === "bn" ? "প্রতিষ্ঠানের নাম" : "Company name"}</label><input id="co-name" {...register("company_name")} className="input" /></div>
                      <div className="grid grid-cols-2 gap-3"><div><label className="form-label" htmlFor="co-bin">BIN</label><input id="co-bin" {...register("company_bin")} className="input" /></div><div><label className="form-label" htmlFor="co-tin">TIN</label><input id="co-tin" {...register("company_tin")} className="input" /></div></div>
                      <div><label className="form-label" htmlFor="co-po">{lang === "bn" ? "PO / ওয়ার্ক অর্ডার নম্বর" : "PO / work order number"}</label><input id="co-po" {...register("po_number")} className="input" /><p className="text-xs text-muted mt-1">{lang === "bn" ? "সরকারি ক্রয়ের বিল PO নম্বর ছাড়া পরিশোধ হয় না।" : "Government purchases are not settled without it."}</p></div>
                      <div><label className="form-label" htmlFor="co-billing">{lang === "bn" ? "বিলিং ঠিকানা (ভিন্ন হলে)" : "Billing address (if different)"}</label><textarea id="co-billing" {...register("billing_address")} rows={2} className="input resize-none" /></div>
                    </div>
                  </div>
              ...
              <section className="enterprise-card p-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {paymentOptions.map((opt) => <label key={opt.id} className={cn("flex items-start gap-3 p-4 rounded-xl border-2 cursor-pointer transition-all enterprise-card-hover", selectedGateway === opt.gateway ? "border-brand-500 bg-brand-50 dark:bg-brand-900/30" : "border-gray-200 dark:border-white/10 hover:border-gray-300 dark:hover:border-white/20")}><input type="radio" value={opt.gateway} {...register("payment_gateway")} className="sr-only" /><span className="text-xl">{opt.icon}</span><div className="flex-1 min-w-0"><div className="flex items-center gap-2"><p className="font-semibold text-sm">{opt.label}</p>{(opt.gateway === "cod" || opt.gateway === "cash_on_delivery") && <span className="badge badge-default text-[10px]">{lang === "bn" ? "জনপ্রিয়" : "Popular"}</span>}</div><p className="text-xs text-muted mt-0.5">{paymentConsequence(opt)}</p>{opt.detail && opt.gateway !== "cod" && opt.gateway !== "cash_on_delivery" && <p className="text-[11px] text-muted mt-0.5 truncate">{opt.detail}</p>}</div>{selectedGateway === opt.gateway && <div className="w-4 h-4 rounded-full bg-brand-500 flex items-center justify-center"><div className="w-1.5 h-1.5 rounded-full bg-white" /></div>}</label>)}
                </div>
