...
export default function SearchSuggestions({ query, onSelect, className }: Props) {
  ...
  return (
    <div ref={ref} className={cn("absolute top-full left-0 right-0 mt-1 glass rounded-xl shadow-glass-strong border border-gray-100 dark:border-white/10 overflow-hidden z-50", className)}>
      {loading ? (
        <div className="p-4 flex justify-center"><Loader2 className="w-5 h-5 animate-spin text-brand-500" /></div>
      ) : items.length === 0 ? (
          ...
          {items.map((item) => (
            <li key={item.slug}>
              ...
              >
                ...
                <span className="text-accent-600 font-medium">৳{item.price.toLocaleString("bn-BD")}</span>
              </Link>
            </li>
          ))}
          <li>
