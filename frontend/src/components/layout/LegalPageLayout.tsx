    ...
    <div className="container mx-auto px-4 py-10 max-w-5xl">
      <div className="grid lg:grid-cols-[220px_1fr] gap-8 items-start">
        <aside className="hidden lg:block sticky top-[calc(var(--navbar-offset)+1.5rem)]">
          <nav aria-label="Table of contents" className="enterprise-card p-4 space-y-1">
            <p className="text-xs font-semibold uppercase tracking-wider text-muted mb-3 px-2">Contents</p>
            {sections.map((s) => (
              <a
                key={s.id}
                href={`#${s.id}`}
