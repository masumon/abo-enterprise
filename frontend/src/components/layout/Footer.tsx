...
export default function Footer() {
  ...
  const { settings } = usePublicSettings(["whatsapp_number", "contact_phone", "contact_email", "contact_address", "contact_address_en", "contact_hours_en", "contact_hours_bn", "footer_about_en", "footer_about_bn", "facebook_url", "instagram_url", "linkedin_url", "youtube_url", "tiktok_url", "play_store_url", "app_store_url", "trade_license", "footer_payment_image_url", SITE_TRUST_BADGES_KEY, SITE_REGISTRATIONS_KEY]);
  ...
  return (
    <footer className="site-footer relative text-white overflow-hidden bg-gradient-to-b from-brand-700 via-brand-800 to-brand-900">
      ...
      <div className="relative z-10 mx-auto max-w-3xl px-4 py-6 md:py-10"><BrandLogo size="lg" href={false} variant="light" />{aboutText && <p className="text-[13px] text-white/75 mt-3 mb-5 leading-relaxed tracking-[0.01em]">{aboutText}</p>}
        ...
        {(playStoreUrl || appStoreUrl) && <div className="pt-5"><h3 className="text-sm font-bold text-white">{bn ? "আমাদের অ্যাপ ডাউনলোড করুন" : "Download Our App"}</h3><p className="text-[11px] text-white/80 mt-0.5">{bn ? "যেকোনো সময় সহজেই কেনাকাটা করুন।" : "Shop anytime, anywhere."}</p><div className="flex gap-2 mt-3">{playStoreUrl && <button type="button" onClick={() => handleAppDownload(playStoreUrl)} className="flex-1 flex items-center gap-2 rounded-xl bg-white/[0.06] hover:bg-white/10 transition-colors px-3 py-2"><PlayStoreMark className="w-5 h-5 flex-shrink-0" /><span className="text-left leading-tight"><span className="block text-[8px] uppercase tracking-wide text-white/80">{bn ? "পাওয়া যাচ্ছে" : "Get it on"}</span><span className="block text-[12px] font-extrabold text-white">{bn ? "গুগল প্লে" : "Google Play"}</span></span></button>}{appStoreUrl && <button type="button" onClick={() => handleAppDownload(appStoreUrl)} className="flex-1 flex items-center gap-2 rounded-xl bg-white/[0.06] hover:bg-white/10 transition-colors px-3 py-2"><AppStoreMark className="w-5 h-5 flex-shrink-0" /><span className="text-left leading-tight"><span className="block text-[8px] uppercase tracking-wide text-white/80">{bn ? "ডাউনলোড করুন" : "Download on the"}</span><span className="block text-[12px] font-extrabold text-white">{bn ? "অ্যাপ স্টোর" : "App Store"}</span></span></button>}</div></div>}
