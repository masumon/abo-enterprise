...
export default function Footer() {
  ...
  const socialLinks: { href: string; icon: ElementType; label: string; className: string }[] = [
    { href: getSettingValue(settings, "facebook_url", "https://www.facebook.com/abo.enterprise"), icon: Facebook, label: "Facebook", className: "bg-[#1877f2]" },
      ...
      <div className="relative z-10 mx-auto max-w-3xl px-4 py-6 md:py-10">
        ...
        <div className="grid grid-cols-4 gap-2 mb-5">
          <a href={`tel:+${phoneDigits}`} className="flex flex-col items-center gap-1.5 rounded-2xl bg-white/[0.06] py-3 px-1 hover:bg-white/10 transition-colors">
            <span className="w-8 h-8 rounded-xl bg-brand-500/15 text-brand-300 flex items-center justify-center"><Phone className="w-3.5 h-3.5" aria-hidden /></span>
          ...
          {whatsappDigits && (
            <a href={`https://wa.me/${whatsappDigits}`} target="_blank" rel="noopener noreferrer" className="flex flex-col items-center gap-1.5 rounded-2xl bg-white/[0.06] py-3 px-1 hover:bg-white/10 transition-colors">
              <span className="w-8 h-8 rounded-xl bg-green-500/15 text-green-400 flex items-center justify-center"><MessageCircle className="w-3.5 h-3.5" aria-hidden /></span>
          ...
          <a href={`mailto:${emailAddr}`} className="flex flex-col items-center gap-1.5 rounded-2xl bg-white/[0.06] py-3 px-1 hover:bg-white/10 transition-colors">
            <span className="w-8 h-8 rounded-xl bg-brand-500/15 text-brand-300 flex items-center justify-center"><Mail className="w-3.5 h-3.5" aria-hidden /></span>
          ...
          <a href={mapsLink} target="_blank" rel="noopener noreferrer" className="flex flex-col items-center gap-1.5 rounded-2xl bg-white/[0.06] py-3 px-1 hover:bg-white/10 transition-colors">
            <span className="w-8 h-8 rounded-xl bg-accent-500/15 text-accent-300 flex items-center justify-center"><MapPin className="w-3.5 h-3.5" aria-hidden /></span>
        ...
        <div className="rounded-2xl bg-white/[0.06] p-4 mb-5 space-y-3">
          <div className="flex items-start gap-3"><span className="w-7 h-7 rounded-lg bg-accent-500/15 text-accent-300 flex items-center justify-center flex-shrink-0"><Clock className="w-3.5 h-3.5" aria-hidden /></span><div className="min-w-0"><p className="text-[10px] font-bold uppercase tracking-wide text-white/70">{bn ? "কার্যসময়" : "Business Hours"}</p><p className="text-xs font-semibold text-white mt-0.5">{hours}</p></div></div>
          <div className="flex items-start gap-3"><span className="w-7 h-7 rounded-lg bg-accent-500/15 text-accent-300 flex items-center justify-center flex-shrink-0"><MapPin className="w-3.5 h-3.5" aria-hidden /></span><div className="min-w-0"><p className="text-[10px] font-bold uppercase tracking-wide text-white/70">{bn ? "প্রধান কার্যালয়" : "Head Office"}</p><a href={mapsLink} target="_blank" rel="noopener noreferrer" className="text-xs font-semibold text-white/80 hover:text-white mt-0.5 block">{address}</a></div></div>
        </div>

        <div className="flex gap-2 flex-wrap mb-5">
          {socialLinks.map(({ href, icon: Icon, label, className }) => (
            <a key={label} href={href} target="_blank" rel="noopener noreferrer" aria-label={label} className={`w-9 h-9 rounded-xl flex items-center justify-center text-white hover:scale-105 transition-transform ${className}`}><Icon className="w-4 h-4" aria-hidden /></a>
          ))}
        </div>
        ...
        <div className="border-t border-white/10">
          {NAV_GROUPS.map((group) => (
            <details key={group.id} className="group border-b border-white/10">
              <summary className="flex items-center justify-between py-3.5 px-0.5 text-sm font-bold text-white cursor-pointer list-none [&::-webkit-details-marker]:hidden">{bn ? group.title.bn : group.title.en}<ChevronDown className="w-4 h-4 text-white/75 transition-transform group-open:rotate-180 group-open:text-green-400" aria-hidden /></summary>
              <ul className="pb-3 px-0.5 space-y-0.5">{group.links.map((link, i) => (<li key={`${link.href}-${i}`}><Link href={link.href} className="flex items-center gap-2 py-1.5 px-1 rounded-lg text-[13px] text-white/85 hover:text-white hover:bg-white/[0.05] transition-colors"><span className="w-1 h-1 rounded-full bg-white/40" />{bn ? link.label.bn : link.label.en}</Link></li>))}</ul>
            </details>
          ))}
        </div>
        ...
        <div className="pt-5"><h3 className="text-sm font-bold text-white">{bn ? "আমাদের অ্যাপ ডাউনলোড করুন" : "Download Our App"}</h3><p className="text-[11px] text-white/80 mt-0.5">{bn ? "যেকোনো সময় সহজেই কেনাকাটা করুন।" : "Shop anytime, anywhere."}</p><div className="flex gap-2 mt-3"><button type="button" onClick={() => handleAppDownload(playStoreUrl)} className="flex-1 flex items-center gap-2 rounded-xl bg-white/[0.06] hover:bg-white/10 transition-colors px-3 py-2"><PlayStoreMark className="w-5 h-5 flex-shrink-0" /><span className="text-left leading-tight"><span className="block text-[8px] uppercase tracking-wide text-white/80">{bn ? "পাওয়া যাচ্ছে" : "Get it on"}</span><span className="block text-[12px] font-extrabold text-white">{bn ? "গুগল প্লে" : "Google Play"}</span></span></button><button type="button" onClick={() => handleAppDownload(appStoreUrl)} className="flex-1 flex items-center gap-2 rounded-xl bg-white/[0.06] hover:bg-white/10 transition-colors px-3 py-2"><AppStoreMark className="w-5 h-5 flex-shrink-0" /><span className="text-left leading-tight"><span className="block text-[8px] uppercase tracking-wide text-white/80">{bn ? "ডাউনলোড করুন" : "Download on the"}</span><span className="block text-[12px] font-extrabold text-white">{bn ? "অ্যাপ স্টোর" : "App Store"}</span></span></button></div></div>

        <div className="border-t border-white/10 mt-6 pt-5 pb-[calc(var(--mobile-chrome-bottom)+0.5rem)] lg:pb-5 text-center space-y-2.5"><p className="text-xs text-white/85">&copy; {new Date().getFullYear()} ABO ENTERPRISE. {bn ? "সকল অধিকার সংরক্ষিত।" : "All rights reserved."}</p><p className="flex items-center justify-center gap-2 flex-wrap text-xs font-semibold text-white/85"><Link href="/legal/terms" className="hover:text-accent-400 transition-colors">{bn ? "শর্তাবলী" : "Terms & Conditions"}</Link><span className="text-white/30">·</span><Link href="/legal/privacy" className="hover:text-accent-400 transition-colors">{bn ? "গোপনীয়তা নীতি" : "Privacy Policy"}</Link><span className="text-white/30">·</span><Link href="/legal/cookies" className="hover:text-accent-400 transition-colors">{bn ? "কুকি নীতি" : "Cookies"}</Link></p><p className="text-xs text-white/80"><span>{bn ? "তৈরি করেছেন" : "Developed with"}</span>{" ❤️ "}<span>{bn ? "দ্বারা" : "by"}{" "}</span><a href="https://mumainsumon.netlify.app" target="_blank" rel="noopener noreferrer" className="font-semibold text-white hover:text-accent-400 transition-colors">SUMON</a></p></div>
      </div>
    </footer>
  );
}
