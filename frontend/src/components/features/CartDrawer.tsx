...
import { X, Minus, Plus, Trash2, ShoppingBag, ArrowRight, Tag, AlertTriangle, Package } from "lucide-react";
...
export default function CartDrawer() {
  const { items, combos, isOpen, closeCart, updateQuantity, removeItem, updateComboQty, removeCombo, total, comboTotal, stockWarnings, setStockWarnings } = useCartStore();
  ...
  // Coupons discount products only; combos are already bundle-priced (matches
  // the backend's server-side money math), so the combo subtotal is added on
  // top of the product-only figure the coupon was validated against.
  const cartSubtotal = total();
  const comboSubtotal = comboTotal();
  const discount = appliedCoupon?.discountAmount ?? 0;
  const delivery = 0;
  const tax = 0;
  const cartTotal = cartSubtotal - discount + comboSubtotal + delivery + tax;
    ...
    <>
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100/80 gradient-brand">
          <div>
            ...
            <p className="text-white/60 text-xs mt-0.5">
              {items.length + combos.length} {lang === "bn" ? "টি পণ্য" : "items"}
            </p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3" aria-live="polite">
          ...
          {combos.map((c) => (
            <div key={c.combo_id} className="flex gap-4 p-3.5 rounded-2xl border border-accent-200/70 dark:border-accent-500/25 bg-accent-50/60 dark:bg-accent-500/10">
              <div className="w-16 h-16 rounded-xl bg-gradient-to-br from-brand-600 to-brand-800 flex items-center justify-center flex-shrink-0 overflow-hidden">
                {c.image_url ? (
                  <Image src={c.image_url} alt={lang === "bn" ? c.title_bn : c.title_en} width={128} height={128} quality={85} className="object-cover w-full h-full" />
                ) : (
                  <Package className="w-7 h-7 text-accent-400" aria-hidden />
                )}
              </div>
              <div className="flex-1 min-w-0">
                ...
                <p className="text-accent-500 font-bold mt-0.5 text-sm">{formatPrice(c.price)}{c.free_delivery ? <span className="ml-1.5 text-[10px] font-bold text-emerald-600 dark:text-emerald-400">{lang === "bn" ? "ফ্রি ডেলিভারি" : "Free delivery"}</span> : null}</p>
                <div className="flex items-center gap-2 mt-2">
                  <button type="button" onClick={() => updateComboQty(c.combo_id, c.quantity - 1)} className="w-7 h-7 bg-gray-100 dark:bg-white/10 hover:bg-brand-50 dark:hover:bg-white/15 border border-gray-200 dark:border-white/10 rounded-lg flex items-center justify-center" aria-label="Decrease quantity">
                    <Minus className="w-3 h-3" />
                  </button>
                  <span className="w-8 text-center text-sm font-bold text-gray-700 dark:text-gray-200">{c.quantity}</span>
                  <button type="button" onClick={() => updateComboQty(c.combo_id, c.quantity + 1)} className="w-7 h-7 bg-gray-100 dark:bg-white/10 hover:bg-brand-50 dark:hover:bg-white/15 border border-gray-200 dark:border-white/10 rounded-lg flex items-center justify-center" aria-label="Increase quantity">
                    <Plus className="w-3 h-3" />
                  </button>
                  <button type="button" onClick={() => removeCombo(c.combo_id)} className="ml-auto w-7 h-7 text-gray-300 hover:text-red-400 hover:bg-red-50 rounded-lg flex items-center justify-center" aria-label="Remove combo">
                    <Trash2 className="w-3.5 h-3.5" />
                  </button>
                </div>
              </div>
