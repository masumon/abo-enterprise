# SUMONIX AI - Database Migrations (Windows PowerShell)
...
$env:PGPASSWORD = if ($env:POSTGRES_PASSWORD) { $env:POSTGRES_PASSWORD } else { "sumonix_dev" }
$pgHost = if ($env:POSTGRES_HOST) { $env:POSTGRES_HOST } else { "localhost" }
$pgPort = if ($env:POSTGRES_PORT) { $env:POSTGRES_PORT } else { "5432" }
$pgUser = if ($env:POSTGRES_USER) { $env:POSTGRES_USER } else { "sumonix" }
$pgDb = if ($env:POSTGRES_DB) { $env:POSTGRES_DB } else { "sumonix_ai" }
